...
Route::post('v2/cancelservices', function()
{
    ...
    // recupera tickets de servicios que no estan completados
    DB::query("UPDATE ticket_tickets SET status = 0 WHERE status = 2 AND ticket in (SELECT user_card_reference FROM services WHERE pay_type = 3 AND status_id = 7)");

    DB::query("UPDATE services SET user_card_reference = '' WHERE pay_type = 3 AND status_id > 5");
...
Route::post('service/ticket', array('uses' => 'service@ticket'));
//Agendamientos
//Agendamiento
//Crear agendamiento
