...
class ServicesV2_Controller extends Base_Controller {
	...
	public function action_get_driver()
	{
        ...
        $id_city = DB::table('cities_customers')->where('customer_id','=',$idcusto)->first();
        

        $waiting_drivers = Driver::where('city_id','=',$id_city->city_id)->get();

		//dd($waiting_drivers);
        $avaiables = DB::table('drivers')->where('available','=','1')->where('city_id','=',$id_city->city_id)->count();
        $notavaiables = DB::table('drivers')->where('available','=','0')->where('city_id','=',$id_city->city_id)->count();
