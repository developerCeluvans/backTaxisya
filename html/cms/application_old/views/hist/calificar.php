...
mail($para, $titulo, $mensaje, $cabeceras);
    ...
    if($r=='true'){
      $cons="update drivers set available='0',status='NULL' where id='$i'";
    mysql_query($cons);
    }else{
      if($e=='true'){
        ...
        if($p=='true'){
          $cons="update drivers set status='FALSE' where id='$i'";
          mysql_query($cons);
  ...
  echo "
    <script>
      alert('$mensaje');
      parent.window.location='http://www.taxisya.co/dev/cms/public/dashboard';
    </script>
  ";
