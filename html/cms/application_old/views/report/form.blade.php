...
<!--subgroup-->
<p><h3>Se podrán generar reportes de servicio.</h3></p>
...
<div class="row-fluid">
    ...
    <label style="text-transform: initial;"> {{$row['title']['0']}} <small> {{$row['title']['1']}} </small> </label>   
    <div class="section">
        <div >
            <?php
            switch ($row['type']) {
...
//public static function file($name, $attributes = array())
        ...
        <?php echo (isset($row['fileObjName'])) ? "'fileObjName' :" . $row['fileObjName'] . "" : ''; ?>
                ...
                case 'blank_div':
                    //public static function submit($value = null, $attributes = array())
                    echo "<div id='" . $row['attributes']['id'] . "'></div>";
                    break;
                case 'formCloning':
                    ?>
                    <div id="">
...
<div class="section last">
    <div>
        ...
        <a class="btn" id="{{$section}}-{{$idToEdit}}-{{$action}}" >Generar</a><!--onclick='<?php echo $uploadOptions; ?>formPoster(this.id);-->
    </div>
</div>
</div>
<script type="text/javascript">
