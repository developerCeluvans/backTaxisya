...
<div id="UITab" style="position:relative;">
    ...
    <!-- <a class="btn  btn-large btn-danger DeleteAll" href="javascript:void(0)"><i class="icon-trash"></i> Delete  All</a> -->
                ...
                <script type="text/javascript">
                            ...
                            exportJson[{{$tabData['id']}}] = '{"section":"<?php echo $section; ?>","tabData":"{{$tabData['id']}}","action":"export","data":{{json_encode($manageBtns['exportData'])}} }';</script>
                <a class="btn  btn-large " href="javascript:jsonPoster(exportJson[{{$tabData['id']}}])"><i class="icon-plus"></i>Exportar tabla</a>
...
<table class="table table-bordered table-striped" id="dataTable">
    <thead align="center">
        <tr>
            ...
            @foreach($titles as $colNum=>$colTitle)
            @if(substr($colNum, 0, 5)=='comb_' || substr($colNum, 0, 5) =='imgp_')
            <th>{{substr($colNum,5)}}</th>
            @else
            <th>{{$colNum}}</th>
            @endif
    ...
    <tbody align="center">
        ...
        <tr>
            ...
            <td>{{$combResult}}</td>
            @else
            @if(substr($colNum, 0, 5)=="imgp_")
            <td><img src="../../{{$item->$colTitle}}" width="50" height="50" alt="Fotografía de conductor"/></td>
            @else
            <?php if ($item->$colTitle == NULL) { ?>
                <td></td>
            <?php } else { ?>
