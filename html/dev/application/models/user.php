...
/*            
            ...
            if (strlen($service->driver->uuid) > 64) { // android
                //dd("driver android");
                $androidDevices[] = $service->driver->uuid;
                //$result = $push->drivers($androidDevices, PushType::message($push_type), 1, 'default', 'Open', $data
                $result = $push->drivers($androidDevices, PushType::USER_CANCELED_SERVICE, 1, 'default', 'Open', $data);
               //   $result = $push->drivers($androidDevices, PushType::message($push_type), 1, 'honk.wav', 'Open', $data);
            }
            else { // ios
                //dd("driver iOS");
                $result = $push->taxistas_ios($service->driver->uuid, PushType::USER_CANCELED_SERVICE, 1, 'honk.wav', 'Open', $data);
            }
*/
//            Notifier::driver(array($service->driver), PushType::USER_CANCELED_SERVICE, $payload);
            $androidDevices[] = $service->driver;
            Notifier::driver($androidDevices, PushType::USER_CANCELED_SERVICE, $payload);
