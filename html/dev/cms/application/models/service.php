...
class Service extends Eloquent {
    ...
    public $includes = array('complains', 'driver', 'user', 'score', 'kind', 'car', 'state');
    ...
    public function complains() {
        return $this->has_many('Complain', 'service_id');
    }
    ...
    public function requestService($id = false, $lat, $lng, $serviceId = false) {
        list($lat1, $lat2, $lon1, $lon2) = $this->getBoundingBox($lat, $lng, 6, $milesResult = false);
                                              ...
                                              (3959 * ACOS( COS( RADIANS( ? ) ) * 
                                                COS( RADIANS( crt_lat ) ) * COS( RADIANS( crt_lng ) - 
                                                  RADIANS( ? ) ) + SIN ( RADIANS( ? ) ) * SIN( RADIANS( crt_lat ) ) )
                                                                                  ...
                                                                                  WHERE available =  '1'
                                                                                  HAVING distance < ?
                                                                                  ORDER BY distance ",array($lat,$lng,$lat,6));
        // $this->closeDrivers = Driver::where('crt_lat', '>', $lat1)
        //                 ->where('crt_lat', '<', $lat2)
        //                 ->where('crt_lng', '>', $lon1)
        //                 ->where('crt_lng', '<', $lon2)
    ...
    public function getNearServices($lat, $lng, $boundingBox = true) {
        list($lat1, $lat2, $lon1, $lon2) = $this->getBoundingBox($lat, $lng, 1, $milesResult = false);
        $this->bounding['lat1'] = $lat1;
        $this->bounding['lng1'] = $lon1;
        $this->bounding['lat2'] = $lat2;
        $this->bounding['lng2'] = $lon2;
...
//        and available=1;";
        ...
        /* $this->closeServices = DB::table('services')
          ...
          ->get(array('id', 'index_id', 'comp1', 'comp2', 'no', 'barrio', 'obs', 'from_lat', 'from_lng')); */
        if ($boundingBox) {
            $this->closeServices = Service::where('from_lat', '>', $lat1)
                            ->where('from_lat', '<', $lat2)
                            ->where('from_lng', '>', $lon1)
                            ->where('from_lng', '<', $lon2)
