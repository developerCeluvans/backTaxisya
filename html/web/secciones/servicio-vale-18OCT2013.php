        ...
        <div id="tabs">
          ...
          <div id="pagina-1" class="ui-tabs-panel">
            <form name="formuno" method="post" action="" class="pedir-taxi">
              <fieldset>
                <legend>Paso <span class="rojo">2</span> &iquest;D&oacute;nde te recogemos?</legend>
                    <div class="clear"></div>
...
<p class="num-tel">
              ...
              <p class="empresa">
                ...
                <select id="empresa" name="data[select]" class="cform">
                  <option value="0">Seleccione</option>
                  <?php $empresas = DbHandler::GetAll('SELECT titulo FROM empresas ORDER BY titulo ASC');
                  $contadorEmp = count($empresas);
                  for($t=0; $t<$contadorEmp; $t++):
                      ?> <option value="<?= utf8_encode($empresas[$t]['titulo']); ?>"><?= utf8_encode($empresas[$t]['titulo']); ?></option><?php
                  endfor;
                  ?>
                </select>
              </p>
              <div class="clear"></div>
              <p class="vale">
                <label for="select">N&uacute;mero de vale</label>
                <input id="vale" name="" type="text" value="" />
              </p>
               <input name=""  id="tutaxi" onclick="return false" type="button" class="solicitar-servicio-2" value="Solicitar servicio" />
               
                  <a id="okmesagge" href="#mensaje-enviado" class="login-window"></a>
               
              <div id="mensajeerror" class="mensajeerror2" >Recuerde que los campos de direcci&oacute;n,  nombre, tel&eacute;fono, email, empresa y n&uacute;mero de vale son abligatorios.</div>
              <div id="mensajeerroremail" class="mensajeerroremail2" >La direcci&oacute;n de correo electr&oacute;nico es invalida</div>
...
//    var barrio = $('#barrio').val();
    ...
    var direccioncompleta = dir+' '+dirnum+letra1+' # '+dirnum2+letra2+' - '+dirnum3+letra3+' - '+bti2+' - '+aptofi2;
  if($("#check").is(':checked')){
      var servicios = 'Es un servicio contratado por horas';
  }else{
      var servicios = 'No es un servicio por horas';
  }
    if(barrio == 0 || dir=='' || dirnum2=='' || dirnum3=='' || nombre == '' || tel == '' || empresa=='' || vale==''){
        indicador = false;
    }
...
//   }
   ...
   if(empresa==''){
       messageError+='No ha indicado Empresa<br />';
   }
   if(vale==''){
       messageError+='No ha indicado N&uacute;mero de vale<br />';
   }
    if(email==''){
        indicador=false;
    }else{
    ...
    if(!indicador){
        ...
        $.post('controllers/serviciovale.php', { nombre: nombre, email: email, tel: tel, dir: direccioncompleta, serv: servicios, ntaxis: ntaxis, nomenclatura: nomenclatura, empresa: empresa, vale:vale } , function(datos) {
